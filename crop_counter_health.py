    ...
    if os.path.isdir(image_path):
        ...
        if format == 'tif':
            ...
            for channel in channels:
                ...
                if channel_tensor.shape[0] > 1:
                    # Convert RGB to grayscale if needed
                    channel_tensor = channel_tensor[0].unsqueeze(0)
...
def main():
    ...
    parser.add_argument('--output', type=str, default='crop_analysis_results.png',
                        help='Output file path for the visualization')
    parser.add_argument('--min-area', type=int, default=50,
                        help='Minimum area (in pixels) for a region to be considered a plant')

    args = parser.parse_args()

    # Load image
    print(f"Loading image from {args.image}...")
    image = load_multispectral_image(args.image, format=args.format)
    ...
    filtered_regions = [r for r in regions if r['area'] >= args.min_area]
    print(f"Found {len(filtered_regions)} plants (after filtering by minimum area of {args.min_area} pixels)")
